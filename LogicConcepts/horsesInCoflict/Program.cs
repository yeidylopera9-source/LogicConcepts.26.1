...
do
{
    ...
    foreach (var caballoActual in caballos)
    {
        var conflictos = new List<string>();

        foreach (var otroCaballo in caballos)
        {
            if (caballoActual == otroCaballo) continue;

            if (EstanEnConflicto(caballoActual, otroCaballo))
            {
                conflictos.Add(otroCaballo);
            }
        }

        // 4. Mostrar resultados con el formato solicitado
        string resultadoConflictos = conflictos.Count > 0
            ? "Conflicto con " + string.Join(" ", conflictos)
            : "ninguno";

        Console.WriteLine($"Analizando Caballo en {caballoActual} => {resultadoConflictos}");
    }
